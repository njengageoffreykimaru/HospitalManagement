...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Add'])) {
    ...
    if ($id !== "" && $group !== "" && $status !== "") {
        ...
        $sql = "INSERT INTO bankb (id, `group`, `status`) VALUES ('$id', '$group', '$status')";
...
} elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['exit'])) {
    // Redirect or perform any action when the "Exit" button is clicked
    exit();
}
?>
...
<div class="sidebar">
    ...
    <div class="form">
        <form action="" method="post">
            <h2>Add Bank Blood</h2>
            <div class="column">
            <input type="number" name="id" id="" placeholder="Enter ID" required>
            <input type="text" name="group" id="" placeholder="Enter Group" required>
            <input type="text" name="status" id="" placeholder="Enter Status" required>
            </div>
